...
<body>
    ...
    <div class="choose-us-area pt-70 pb-70" id="page" style="background-color:#32A195;">
        <div class="text-center">
            ...
            <h2 style="color:white; text-transform: uppercase;">{{$bppup->kategori}}</h2>
        </div>
        <div class="card container">
            <div class="row">
                <div class="col-lg-12 mb-30">
                    <div class="section-title">
                        <h4 style="padding-top:10px;">{{$bppup->judul}}<br><a href="#"></a></h4>
                    </div>
                    <?php echo $bppup->updated_at ?>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-9">
                    <div class="textartikel">
                        <img style="max-width:50%;" src="{{url('/BPPUPS/'.$bppup->gambar) }}">
                        <div><br><br>
                        <p style="text-">{!!$bppup->isi!!}</p>
                        </div><br>
                        <iframe width="320" height="315" src="{{$bppup->linkyt}}" frameborder="0" allowfullscreen></iframe>
                    </div>
                    
                </div>
                <div class="col-lg-3">
                    <div class="single-choose-item text-center" class="listevent">
                        <h6><a href="#"><i class="fas fa-clipboard"></i> Event</a></h6>
                    </div>
                    @foreach($event as $e)
                    <div class="single-choose-item card">
                        <h6><a href="#">{{$e->nama}}</a></h6>
                        <a href="{{$e->googleform}}" class="btn btn-succes">Daftar segera</a>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <!-- Start Komentar -->
    <div class="choose-us-area black-bg pt-5 pb-5" id="page">
        <div class="text-center">
            <h4 style="color:white;">Masukkan Komentar</h4>
        </div>
        <div class="container">
            <form action="{{ url('komentar/proses/'.$bppup->id) }}" method="POST">
                {{ csrf_field() }}
                        <input type="text" placeholder='Nama' name="nama" style="width:100%; margin-bottom:3px;" required>
                        <div class="form-group">
                        <textarea class="form-control" name="isi" id="exampleFormControlTextarea1" rows="3" placeholder="Masukkan Komentar anda" required></textarea>
                        </div>
                        <input class="btn btn-primary" type="submit" value="Kirim" style="margin-bottom:10px; width:auto;">
            </form>
        </div>
    </div>
    <!-- End komentar -->
    ...
    <div class="choose-us-area black-bg pt-5 pb-5" id="page">
        <div class="text-center">
            <h4 style="color:white;">Komentar</h4>
        </div>
        <div class="container">
        @foreach($bppup->komentar as $k)
            <div class="card">
                        <p><b style="margin-left:5px;">{{$k->nama}}</b> : {{$k->isi}} </p>
            </div>
        @endforeach
            <div class="card">
                        <p><b style="margin-left:5px;">Marina Sianturi</b> : Semua nya menarik dan sangat bagus sekali isinya dan mudah dipahami</p>
            </div>
        </div>
    </div>
    <!-- End komentar -->
